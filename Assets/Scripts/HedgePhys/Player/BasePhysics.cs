...
public class BasePhysics : MonoBehaviour, IDataPersistence
{
    ...
    public float AirControlAmmount = 1.7f;
    ...
    public float RollingLandingBoost=2;
    public float RollingDownhillBoost=2;
    public float RollingUphillBoost=1;
    public float RollingStartSpeed=10;
    public float RollingTurningDecreace=0.5f;
    ...
    {
        //Rolling
        if (Input.GetButton("Roll") && rigidbody.velocity.sqrMagnitude > RollingStartSpeed)
        {
            isRolling = true;
            Debug.Log("Spin");
        }
    ...
    void GeneralPhysics()
    {
        ...
        if (UseSphereToGetNormal)
        {
            ...
            if (Physics.Raycast(transform.position, -CollisionPointsNormal, out hit, RayToGroundDistance, colLay))
            {
        ...
        {
            if (Physics.Raycast(transform.position, -transform.up, out hit, RayToGroundDistance, colLay))
            {
                GroundNormal = hit.normal;
                Grounded = true;
                GroundMovement();
    ...
    public void StickToGround(float StickingPower)
    {
        ...
        if (Physics.Raycast(CollisionPoint.position, -Colliders.up, out hit, GroundStickingDistance) && !Input.GetKey(KeyCode.Space))
        {
